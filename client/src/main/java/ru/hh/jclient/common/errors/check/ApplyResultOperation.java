...
public class ApplyResultOperation<T> extends AbstractOperation<T, ApplyResultOperation<T>> {
   ...
   * </li>
   * <li>{@link ResultWithStatus#uncheckedResult()} contains {@link Optional#empty()} if status is not allowed</li>
   * </ul>
   * </p>
   * <p>
  ...
  /**
   * <p>
   * Returns result or throws {@link WebApplicationException} with provided status code if {@link ResultWithStatus#uncheckedResult()} contains
   * {@link Optional#empty()}.
   * </p>
   * <p>
   * If default value is specified, it will be returned instead of exception.
  ...
  public Optional<T> onPredicate() {
    return checkForPredicates(wrapper.uncheckedResult().orElse(null));
  }

  public CheckedResultWithStatus<T> onPredicateWrapped() {
    return new CheckedResultWithStatus<>(checkForPredicates(wrapper.uncheckedResult().orElse(null)).orElse(null), wrapper.getStatusCode());
  }
}
